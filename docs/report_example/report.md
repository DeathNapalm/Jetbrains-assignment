...
Generated by `scripts/performance_reporting/parse_results.py` from latest simulation logs.

![Requests over time](requests_over_time.svg)

## Request table (transaction aggregation)

Source: [latency_summary.csv](latency_summary.csv) (generated by `scripts/performance_reporting/parse_results.py`).
...
SLO compliance check (based on measured p90 values):
- GET /api/issue/{id}?fields=... <= 500 ms: PASS (mapped to "Open Random Issue", worst p90 = 8.30 ms).
- POST /api/issuesGetter?fields=... <= 1000 ms: PASS (mapped to "Perform Search", p90 = 10.30 ms).
- GET /api/sortedIssues <= 500 ms: PASS (p90 = 143.00 ms, below the 500 ms SLO threshold).
